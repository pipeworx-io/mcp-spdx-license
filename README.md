...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/list_licenses \
  -H 'Content-Type: application/json' \
  -d '{"osiApproved":true}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/list_licenses`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
